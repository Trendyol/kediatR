...
internal class RegistryImpl(
  ...
  override fun <TRequest : Request<TResult>, TResult> resolveHandler(
    ...
    val handler = registry.requestHandlerMap[classOfRequest]?.get()
      ?: registry.requestHandlerMap[baseClassOrItself(classOfRequest, Request::class)]?.get()
  ...
  override fun <TNotification : Notification> resolveNotificationHandlers(
    ...
    .filter { (k, _) -> ReflectionUtils.isAssignableFrom(k, classOfNotification) }
  ...
  private fun baseClassOrItself(
    ...
    generateSequence(clazz) { ReflectionUtils.getSuperclass(it) } // ↪ generate the chain: clazz, clazz.superclass, clazz.superclass.superclass, …
      .filter { ReflectionUtils.isAssignableFrom(clazzWanted, it) } // ↪ keep only those that actually "implement/extend" clazzWanted
