  ...
  ShouldSpec({
    ...
    context("CachedRegistry request handler caching") {
      should("cache request handlers and return same instance on subsequent calls") {
        ...
        val baseRegistry = RegistryImpl(HandlerRegistryProvider(handlers.associateBy { it::class }))
        val cachedRegistry = CachedRegistry(baseRegistry)

        // Act
        val firstCall = cachedRegistry.resolveHandler(TestCommandForWithoutInjection::class)
        val secondCall = cachedRegistry.resolveHandler(TestCommandForWithoutInjection::class)
      ...
      should("cache different handlers for different request types") {
        ...
        // Act
        val resolvedCommandHandler = cachedRegistry.resolveHandler(TestCommandForWithoutInjection::class)
        val resolvedInheritedHandler = cachedRegistry.resolveHandler(TestCommandForInheritance::class)
      ...
      should("propagate exceptions from underlying registry") {
        ...
        val baseRegistry = RegistryImpl(HandlerRegistryProvider(emptyMap()))
        val cachedRegistry = CachedRegistry(baseRegistry)

        // Act & Assert
        var exceptionCount = 0
        try {
          cachedRegistry.resolveHandler(TestCommandForWithoutInjection::class)
    ...
    context("CachedRegistry notification handler caching") {
      should("cache notification handlers and return same collection on subsequent calls") {
        ...
        // Act
        val firstCall = cachedRegistry.resolveNotificationHandlers(NotificationForMultipleHandlers::class)
        val secondCall = cachedRegistry.resolveNotificationHandlers(NotificationForMultipleHandlers::class)
      ...
      should("cache empty collections for notifications with no handlers") {
        ...
        // Act
        val firstCall = cachedRegistry.resolveNotificationHandlers(TestNotification::class)
        val secondCall = cachedRegistry.resolveNotificationHandlers(TestNotification::class)
